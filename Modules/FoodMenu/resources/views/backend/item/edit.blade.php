...
@section('moduleContent')
    ...
    <div><a href="/admin/foodmenu/item">List</a> | <a href="/admin/foodmenu/item/{{$item->id}}">View</a></div>
    <div>
        <form action="{{route('foodmenu.item.update', $item->id)}}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <div>
                    <textarea name="info" id="short_info" rows="2" class="form-control">{{old('info') ?? $item->info}}</textarea>
                </div>
            </div>
            <div class="mb-3">
                <label for="description" class="form-label">Description</label>
                <div>
                    <textarea name="description" id="description" rows="6" class="form-control">{{old('description') ?? $item->description}}</textarea>
                </div>
            </div>
            <div class="mb-3">
                <label for="note" class="form-label">Note</label>
                <div>
                    <textarea name="note" id="note" rows="2" class="form-control">{{old('note') ?? $item->note}}</textarea>
                </div>
            </div>
            <div class="row mb-3">
                <div class="col-sm-12 col-lg-4" id="singlePrice">
                    <div class="border-box">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="single_price" class="form-control" value="{{old('single_price') ?? $item->price}}">
                        </div>
                    </div>
                </div>
                <div class="col-sm-12 col-lg-6" id="multiplePrice">
                    <div class="border-box">
                        ...
                        <div id="multiplePriceList">
                            @if($item->prices)
                                @foreach ($item->prices as $key => $priceItem)
                                    <div class="row mb-3 item">
                                        <div class="col-sm-4 col-lg-4"><input type="text" name="price_label[{{$key}}]" class="form-control" placeholder="Label" value="{{ old('price_label['.$key.']') ?? $priceItem->title }}"></div>
                                        <div class="col-sm-8 col-lg-8" style="position:relative; padding-right:30px">
                                            <div class="input-group">
                                                <span class="input-group-text" id="basic-addon1">{{_currencySymbol()}}</span>
                                                <input type="number" name="multi_price[{{$key}}]" id="price{{$key}}" class="form-control"  value="{{old('multi_price['.$key.']') ?? $priceItem->price}}" >
                                            </div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-8 col-lg-8">
                                    <div class="input-group">
                                        <span class="input-group-text" id="basic-addon1">{{_currencySymbol()}}</span>
                                        <input type="number" name="multi_price[]" id="price1" class="form-control" placeholder="0.00" value="{{old('multi_price[0]')}}" >
                                    </div>
                                </div>
                            </div>
                            @endif
            ...
            <div class="row mb-3">
                <div class="col-sm-12 col-lg-6">
                    <div class="row">
                        <div class="col-lg-2">
                            <div class="image" style="width: 100px; min-height:100px; background:#eee">
                                @if($item->thumb)<div><img src="{{url('storage/images/'.$item->thumb)}}" alt="{{$item->title}}" style="width:100%"></div>@endif
                            </div>
                        </div>
                        <div class="col-lg-10">
                ...
                <div class="col-sm-12 col-lg-6">
                    <div class="row">
                        <div class="col-lg-2">
                            <div class="image" style="width: 100px; min-height:100px; background:#eee">
                                @if($item->cover_photo)<div><img src="{{url('storage/images/'.$item->cover_photo)}}" alt="{{$item->title}}" style="width:100%"></div>@endif
                            </div>
                        </div>
                        <div class="col-lg-10" class="form-label">
        ...
        const priceField = '<div class="row mb-3 item">'
                            ...
                            +'<div class="col-sm-8 col-lg-8" style="position:relative; padding-right:30px">'
                                +'<div class="input-group">'
                                    ...
                                    +'<input type="number" name="multi_price[]" class="form-control" placeholder="0.00" >'
                                +'</div>'
                                +'<button type="button" class="remove-price-item btn btn-sm btn-outline-default" style="position:absolute; right:0; top:5px">X</button>'
                            +'</div>'
                        +'</div>';
